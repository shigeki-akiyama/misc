...
if (me % 2 == 1) {
#if 1
    size_t max_bufs = 10;
    std::vector<message> bufs(max_bufs);
    std::vector<MPI_Request> reqs(max_bufs);

    for (i = 0; i < max_bufs; i++) {
        MPI_Irecv(&bufs[i], sizeof(message), MPI_BYTE, MPI_ANY_SOURCE, 0,
                  MPI_COMM_WORLD, &reqs[i]);
    }
    for (;;) {
        int idx;
        MPI_Waitany(max_bufs, reqs.data(), &idx, MPI_STATUS_IGNORE);

        MPI_Send(&bufs[idx], sizeof(message), MPI_BYTE, bufs[idx].pid, 0,
                 MPI_COMM_WORLD);

        MPI_Irecv(&bufs[idx], sizeof(message), MPI_BYTE, MPI_ANY_SOURCE, 0,
                  MPI_COMM_WORLD, &reqs[idx]);
    }
#else
    for (;;) {
        MPI_Recv(&msg, sizeof(msg), MPI_BYTE, MPI_ANY_SOURCE, 0,
                 MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Send(&msg, sizeof(msg), MPI_BYTE, msg.pid, 0,
                 MPI_COMM_WORLD);
    }
#endif
} else {
    ...
    for (i = 0; i < n_msgs; i++) {
        ...
        MPI_Send(&msg, sizeof(msg), MPI_BYTE, target, 0, MPI_COMM_WORLD);
        ...
        MPI_Recv(&msg, sizeof(msg), MPI_BYTE, target, 0, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE);
